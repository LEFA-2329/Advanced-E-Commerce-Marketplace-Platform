...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        if ($edit_mode) {
            $stmt = $pdo->prepare("UPDATE products SET name = ?, description = ?, price = ?, stock_quantity = ?, category = ?, image_url = ?, updated_at = NOW() WHERE product_id = ? AND owner_id = ?");
            $stmt->execute([$name, $description, $price, $stock_quantity, $category, $image_url, $product_id, $owner_id]);
        } else {
            $stmt = $pdo->prepare("INSERT INTO products (name, description, price, stock_quantity, category, image_url, owner_id) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$name, $description, $price, $stock_quantity, $category, $image_url, $owner_id]);
        }
        header("Location: product_management.php");
        exit;
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        if ($edit_mode) {
            $stmt = $pdo->prepare("UPDATE products SET name = ?, description = ?, price = ?, stock_quantity = ?, category = ?, image_url = ?, updated_at = NOW() WHERE product_id = ?");
            $stmt->execute([$name, $description, $price, $stock_quantity, $category, $image_url, $product_id]);
        } else {
            $stmt = $pdo->prepare("INSERT INTO products (name, description, price, stock_quantity, category, image_url) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$name, $description, $price, $stock_quantity, $category, $image_url]);
        }
        header("Location: product_management.php");
        exit;
    } else {
