...
try {
    ...
    // Log the script execution to file instead of database
    $logMessage = "[" . date('Y-m-d H:i:s') . "] Order status update script executed. Updated $updatedCount orders.\n";
    file_put_contents(__DIR__ . '/order_update_log.txt', $logMessage, FILE_APPEND);

} catch (Exception $e) {
    echo "Error: " . $e->getMessage() . "\n";

    // Log the error to file instead of database
    $errorMessage = "[" . date('Y-m-d H:i:s') . "] Order status update script error: " . $e->getMessage() . "\n";
    file_put_contents(__DIR__ . '/order_update_error_log.txt', $errorMessage, FILE_APPEND);
    echo "Error logged to order_update_error_log.txt\n";
}
